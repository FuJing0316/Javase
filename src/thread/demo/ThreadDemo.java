 ...
 *      2、就绪状态：准备开始执行，但并没有执行（调用start方法之后）
 ...
 *          进入的方式：
 *              sleep方法（使当前正在执行的线程休眠一段时间，进入阻塞状态）
 *              等待io资源
 *              join方法（抢占优先执行的机会:调用join方法的线程，会优先执行，当前正在执行的线程会被阻塞）
 *
 *      注意：
...
public class ThreadDemo extends Thread {
    ...
    public static void main(String[] args) {
        //每次运行以下相同代码片段，可能执行结果不同，原因是多线程中谁先抢占到cpu资源，无法人为控制
        for (int i = 0; i < 5; i++) {
            ThreadDemo threadDemo = new ThreadDemo();//线程子类
            threadDemo.start();
            System.out.println("主线程名称，是否优先执行了？"+Thread.currentThread().getName()+"==========="+i);
        }
    }
}

 ...
 *
 * 注意：
 * 1.以上提到的几种基本类型包装类均实现了常量池技术，但他们维护的常量仅仅是【-128至127】这个范围内的常量，如果常量值超过这个范围，
 * 就会从堆中创建对象，不再从常量池中取。比如，把上边例子改成Integer i1 = 400; Integer i2 = 400;
 * 很明显超过了127，无法从常量池获取常量，就要从堆中new新的Integer对象，这时i1和i2就不相等了。
 *
 * 2、String类也实现了常量池技术，但稍微有所不同。String是先检测常量池中是否相应的字符串，如果有，则取出来。 如果没有，则把当前的添加进去。
 *
 *
 *
 * -------------------------------------------------------
